<template>
  <modals-modal v-model="show" name="batchQuickMatchCovers" :processing="processing" :width="500" :height="'unset'" @submit="doBatchQuickMatchCovers">
    <template #outer>
    ...
    <div ref="container" class="w-full rounded-lg bg-bg box-shadow-md overflow-y-auto overflow-x-hidden" style="max-height: 80vh">
      <div v-if="show" class="w-full h-full py-4">
        <div class="w-full overflow-y-auto overflow-x-hidden max-h-96">
          ...
          <p class="text-base px-8 py-2">Search the selected provider for covers and apply them to the selected items. This will not overwrite any other metadata.</p>
          ...
          <div class="mt-4 pt-4 text-white/80 border-t border-white/5">
            <div class="flex items-center px-4">
              ...
              <div class="grow" />
              <ui-btn color="bg-success" @click="doBatchQuickMatchCovers">{{ $strings.ButtonSubmit }}</ui-btn>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  computed: {
    show: {
      get() {
        return this.$store.state.globals.showBatchQuickMatchCoversModal
      },
      set(val) {
        this.$store.commit('globals/setShowBatchQuickMatchCoversModal', val)
      }
    },
    title() {
      return this.$getString('MessageItemsSelected', [this.selectedBookIds.length])
    },
    showBatchQuickMatchCoversModal() {
      return this.$store.state.globals.showBatchQuickMatchCoversModal
    },
    selectedBookIds() {
  ...
  methods: {
    ...
    doBatchQuickMatchCovers() {
      ...
      this.$axios
        ...
        .then(() => {
          this.$toast.info(`Batch Quick Match Covers started for ${this.selectedBookIds.length} items`)
        })
        .catch((error) => {
          this.$toast.error('Batch Quick Match Covers failed')
          console.error('Failed to batch quick match covers', error)
        })
        .finally(() => {

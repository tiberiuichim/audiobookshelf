...
- **Duplicate Handling**: If the target location already has a folder with the same name, a prompt allows merging the items. True duplicate files are safely skipped, and the destination item is automatically updated to include any new files.

...
This feature targets the **main library bookshelf** (`/library/:id/bookshelf`) and the **library home page** (`/library/:id`). It does not apply to series, collections, playlists, authors, genres, or tags pages — those entities use their own card layouts.
...
| `client/components/app/BookShelfCategorized.vue` | Home page categorized shelves | Add conditional list view rendering path |
...
The `LazyBookshelf` component currently renders cards using the `bookshelfCardsHelpers` mixin with virtual scrolling (shelves + entity mounting). For the List view, a simpler rendering approach is appropriate since list rows are much shorter and don't need the complex shelf-based layout.

**Approach:** When `isListView` is true, render a `<div>` containing `BookListRow` components for each loaded entity, using a standard `v-for` with virtual scrolling. The existing pagination (`fetchEntites`, `loadPage`) remains the same — only the rendering changes.

**Template changes:**

```html
<!-- List view rendering -->
<div v-if="isListView && initialized && entities.length" class="w-full list-view-container">
...
**Important:** The list view should still use the same data fetching (`fetchEntites`, `loadPage`, pagination) — only the presentation layer changes.

### 5. BookShelfCategorized — Home Page List View

**File:** `client/components/app/BookShelfCategorized.vue`

The home page already has two rendering paths: `isAlternativeBookshelfView` (using `widgets-item-slider`) and the standard bookshelf (using `app-book-shelf-row`). The list view should be a third path.

When `isListView` is true on the home page, each shelf category (Continue Listening, Recently Added, etc.) renders its entities as list rows instead of cards. This can be achieved by:

1. Adding a new template branch for list view
2. For each shelf, rendering `BookListRow` components in a vertical stack
3. Shelf category headers remain as section dividers

```html
<div v-else-if="isListView" class="w-full mb-24e">
  <template v-for="(shelf, index) in supportedShelves">
    <div :key="index" class="mb-6e">
      <h2 class="font-semibold text-gray-100 px-4e">{{ $strings[shelf.labelStringKey] }}</h2>
      <template v-for="(entity, entityIndex) in shelf.entities">
        <tables-library-book-list-row
          :key="entity.id"
          :library-item="entity"
          :index="entityIndex"
          :book-cover-aspect-ratio="coverAspectRatio"
          :continue-listening-shelf="shelf.id === 'continue-listening'"
          @select="(payload) => selectEntity(payload, index)"
          @edit="editItem"
        />
      </template>
    </div>
...
| `client/components/app/BookShelfCategorized.vue` | Modify | Add list view rendering path for home page |
...
1. **Toggle button visibility**: Confirm the view toggle button appears on the library bookshelf page and home page, but NOT on series/collections/playlists/authors pages.
...
1. **No virtual scrolling optimization for list view**: The initial implementation renders all loaded entities in the DOM. For very large libraries (1000+ items), this could impact performance. Future work could add intersection observer-based virtual scrolling for list rows.
2. **Series/collections/playlists/authors pages unaffected**: The list view toggle only applies to the main library bookshelf and home page. Other entity types use their own card layouts and are not converted to list view.
